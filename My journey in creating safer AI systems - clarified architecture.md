...
Modern agent systems also separate responsibilities across planners, executors, tools, memories, routers, evaluators, and language models. My architecture arrives at the problem from a different direction, but it shares the same dangerous property:

> One component can shape what another component is allowed to see, remember, retrieve, forget, or treat as prior truth.

That is why I believe the findings in this document apply beyond my own system.

I discovered them in a custom runtime/render architecture designed specifically to prevent the LLM from owning the system.

The broader vulnerability belongs to composed AI systems.

Agent wrappers are one important example.

Tool chains are another.

Planner/executor systems are another.

Memory-augmented systems are another.

Multi-stage reasoning and multi-agent systems are another.

Anywhere authority is split, information flow can become authority.

That distinction matters because I did not find these problems by giving an LLM too much control.

I found them after taking control away from the LLM and putting it into the architecture.

That is what made the findings harder to dismiss.

The model was not quietly becoming king.

The architecture was quietly creating kings of its own.
...
Because my runtime owns intermediate state, routing inputs, retention, eligibility, and decision dependencies, I can intervene directly on the software graph around the generative model.

The LLM is not the hidden object I am trying to interpret.

The larger system is the object under test.

That is another reason I think this work is relevant to agent wrappers.

As AI systems become more composed, more of the important causal behavior moves outside the foundation model and into the machinery that surrounds it.
...
This is also where the connection to modern agent wrappers becomes unavoidable.

My own system is a bounded generative runtime with an LLM renderer, not a conventional agent that lets the model own the whole loop.

But agent wrappers inherit the same class of problem whenever authority is divided across components.

A planner can shape what an executor is allowed to do.

A tool adapter can shape what a planner believes happened.

A summarizer can shape what memory later calls history.

A retriever can shape what the model thinks is relevant.

An evaluator can shape which actions remain reachable.

A memory eligibility layer can decide what future reasoning will be allowed to treat as prior truth.

The names differ.

The authority pattern does not.
