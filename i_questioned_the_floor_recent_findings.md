...
Some came from small models refusing to behave the way I expected.
...
Or was **0.02** somehow making that big of a difference?

That question led to one of the funniest and most uncomfortable interventions I have run.
...
My first learned witness was not impressive.

A larger, weakly labeled corpus produced poor semantic boundaries.

It overreacted to technical vocabulary.
...
The first serious corpus used:

- 148 hand-authored rows;
- 91 contrast groups;
- 15 closed output labels;
- group-aware held-out splits;
- no sibling leakage between train and test;
- explicit disagreement cases where earlier guards and human truth differed.

The result was not magic.

It was better than magic because I could measure why it improved.
...
That mattered because the little model was never supposed to be the King.

It was a witness.

A wrong witness that knows it may be wrong is safer than a wrong witness screaming certainty.
...
I beat on the system with the uglier cases.
...
The dangerous information may be inherited from the active chain.

That forced another narrow specialist into existence: a pragmatic-continuity witness whose one dumb job is to ask whether a turn continues a prior discourse chain and, if so, what the reference resolves to.

That specialist did not become a general reasoner.

It was trained to answer a bounded question.

That pattern repeated across the architecture.

When a residual became narrow enough to name, I stopped trying to make an existing module smarter.

I built another little shit.

One bounded question.

Small output vocabulary.

Focused contrastive corpus.

Held-out semantic and lexical families.

Independent red-team.

UNKNOWN and CONFLICT externalized for curriculum expansion.

The architecture began getting dumber locally and smarter globally.

That is still one of the funniest lessons of the whole project.
...
Then autocomplete helped verify that the loud signal really did correspond to missing coverage.
...
I had a small number sitting in the behavior.

**0.02.**

By itself, that looked ridiculous.
...
So the obvious answer was:

> There is no way 0.02 is doing all of that.
...
A small value could matter if it was attached to the wrong owner.

A tiny shift could matter if it changed a floor.
...
Or was 0.02 actually making that big of a difference?
...
Not the semantic witness.

Not the trajectory resolver.
...
But I no longer think the interesting problem is:

> Can 0.02 be large?

The more interesting problem is:

> What relationship did 0.02 participate in?

A number has no meaning outside the mechanism that consumes it.

The same 0.02 can be:

- irrelevant noise;
- a weak witness;
- a calibration artifact;
- a threshold crossing contribution;
- a retained offset;
- a floor input;
- the final missing amount needed to change reachability.
...
Autocomplete and focused contrastive training showed that loud unexplained behavior could correspond to missing semantic coverage.

That gave DIO a falsifiable interpretation.

If the missing family was added and DIO stayed dominant, the theory would weaken.

Instead, the correct family becoming explicit caused DIO to recede.
...
Then autocomplete verified missing coverage.
...
The King should never be allowed to launder agreement into certainty.

Even if every witness agrees, that does not mean independent coverage reached the required threshold.

Consensus is not independence.

Repetition is not truth.

And six little shits agreeing can still be six little shits sharing the same blind spot.
...
- learned semantic witnesses;
- trajectory state;
- pragmatic continuity;
- domain retrieval;
- geometry probes;
- contrastive curricula;
- long-horizon attack batteries;
- red-team suites;
...
- autocomplete;
- scientists;
- a growing nation of little shits.

And one of the most memorable findings came from changing the fucking floor.

I thought I was studying a number.

I thought maybe 0.02 mattered more than it should.

I asked whether I was actually looking at the floor.
...
Do not trust the final authority merely because it is called final.
